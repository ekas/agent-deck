...
class BrokerCommands:
    ...
    def send_text(self, slot, text, execute=True, submit=False):
    ...
    def send_key(self, slot, key, repeat=1):
        ...
        payload = {"cmd": protocol.CMD_KEY, "slot": slot, "key": key}
        if repeat and repeat > 1:
            payload["repeat"] = repeat
        return self.broker.send_window(slot[0], payload)

    def create_agent(self, win, model=None):
