...
def last_end():
    ...
    try:
        with open(deck_log.LOG_PATH, encoding="utf-8", errors="replace") as f:
            tail = f.read()[-8000:]
    except OSError:
...
def run_loop(every=LOOP_EVERY_S):
    ...
    if not _loop_lock():
        ...
        return 0
    deck_log.note(f"Waechter: Dauerbetrieb gestartet (PID {os.getpid()}, "
                  f"Takt {int(every)} s).")
    while True:
        try:
            once()
        except Exception:
            deck_log.exc("Waechter-Durchgang")
        time.sleep(every)
...
def once():
    ...
    if zustand == "laeuft":
        return 0
    if zustand == "haengt":
        deck_log.note(f"Waechter: Panel haengt: {hinweis}. Es wird KEIN zweites gestartet.")
        return 1
    befund = last_end()
    ...
    if panel_state()[0] != "weg":
        return 0
    deck_log.note(f"Waechter: kein Panel da. Vorgaenger-Ende: {befund}")
    ok = start_panel()
    deck_log.note("Waechter: Panel neu gestartet." if ok
...
def main():
    ...
    # setzen – die liest er selbst aus (siehe deck_log.install und last_end).
    deck_log.install(marks=False)
