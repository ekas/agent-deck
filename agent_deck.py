...
from canvas_kit import (BG, CARD_FILL, CARD_BORDER, INK, INK_2, INK_3,
                        hex_to_rgb as _hex_to_rgb, mix as _mix,
                        short_model as _short_model)
from bindstore import (BindStore, is_placeholder_ws as _is_placeholder_ws,
                       repo_from_title as _repo_from_title,
                       ticket_branch as _ticket_branch, ticket_slug as _ticket_slug,
                       jira_key as _jira_key)
from broker import Broker
from broker_commands import BrokerCommands
from glow_animator import GlowAnimator, GLOW_RINGS
...
class AgentDeck:
    def __init__(self):
        ...
        # (siehe hidpi.py). Der Oberflaechenfaktor holt die Groesse zurueck:
        ...
        hidpi.enable()
        hidpi.set_ui(hidpi.system_factor())
        ...
        # hidpi.fontpx(), damit die Kachelschrift exakt dem Kachelraster folgt.
        hidpi.sync_tk_scaling(self.root)
        ...
        deck_log.hook_tk(self.root)  # Tk-Callback-Fehler ins Log statt ins Leere
        ...
        # der Faktor davon ab (Zoom) – der Bezug bleibt aber hidpi.ui().
        self._slim_scale = hidpi.ui()
    ...
    def _apply_slim_layout(self):
        ...
        self.agent_area.pack(side="top", padx=hidpi.px(10),
                             pady=(hidpi.px(6), hidpi.px(4)), fill="both", expand=True)
    ...
    def _sync_ui_scale(self, redraw=True):
        ...
        f = hidpi.factor_for_window(getattr(self, "my_hwnd", 0))
        old = hidpi.ui()
        if abs(f - old) < 0.01:
            return False
        hidpi.set_ui(f)
    ...
    def _seed_slim_size(self):
        ...
        natuerlich heisst hier: eine Design-Einheit = hidpi.ui() Geraetepixel, bei
        ...
        self._render_agents_slim(scale=hidpi.ui())
        self._repaint_tiles()
        self._fit_slim_window(hidpi.ui())
    ...
    def _slim_extent(self):
        ...
        nf.configure(size=hidpi.fontpx(12)[1])
    ...
    def _render_agents_slim(self, scale=None):
        ...
        nf.configure(size=hidpi.fontpx(12, s)[1])
        RING = len(GLOW_RINGS) * 2 * s
        name_h = nf.metrics("linespace")
        small_font = hidpi.fontpx(8, s)
    ...
    def _draw_tile(self, c, slot, x, y, W, H, R, scale=1.0, step=None):
        ...
        s = scale
        # Kachelschrift in PIXELN (hidpi.fontpx): sie folgt damit exakt demselben
        ...
        # liefe aus der Karte.
        fs = lambda b, w=None: hidpi.fontpx(b, s, weight=w)
    ...
    def _tip_at_pointer(self, text):
        ...
        self.prompt_tip.show(self.root.winfo_pointerx(), self.root.winfo_pointery(),
                             text, dx=hidpi.px(14), dy=hidpi.px(18))
    ...
    def _place_dialog(self, dlg):
        ...
        SICHTBAREN Fenster messbar (screen_fit._frame_pad). Der zweite Aufruf aendert
        ...
        screen_fit.place(dlg, *anchor, dx=hidpi.px(30), dy=hidpi.px(60))
        try:
            dlg.deiconify()
        except tk.TclError:
            return
    ...
    def restart(self):
        ...
        deck_log.note("--- Panel-Ende (Neustart, Kind uebernimmt) ---")
        os._exit(0)
    ...
    def run(self):
        ...
        deck_log.note("mainloop beendet (Fenster zerstoert) -> Panel endet regulaer")
...
if __name__ == "__main__":
    ...
    if not si.acquire_or_focus():
        deck_log.note("schon ein Panel da -> dieses tritt zurueck")
        sys.exit(0)
    try:
        AgentDeck().run()
    except BaseException:
        deck_log.exc("Panel-Start/Lauf")
        raise
